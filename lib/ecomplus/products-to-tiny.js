        ...
        if (unwatched.indexOf(result[i]._id || result[i]) === -1) {
          ...
          const promise = store({ url, storeId })
            .then(({ data }) => {
              ...
              return db.fetchProduct(data.sku, storeId)

                .then(row => {
                  if (row && row.length) {
                    // verifica se o produto teve alteração de variação e atualiza o tiny
                    return tiny.fetchProducts(row[0].tiny_id)
                    ...
                    return db.insertProduct(data.sku, storeId, data._id, null, data.name, data.price, data.quantity, 'ecomplus', variations)

                              ...
                              if (retorno && parseInt(status_processamento) < 3) {
                                if (status === 'Erro') {
                                  if (!isNaN(parseInt(codigo_erro)) && parseInt(codigo_erro) === 6) {
                                    // to many requests
                                    console.log('--> Api limit reached, waiting 1m')
                                    console.log('--> Queue size', (rows.length - current))
                                    setTimeout(() => {
                                      // try again
                                      recursiveSync()
                                    }, 60000)
                                ...
                                mysql.updateTinyIdOrder(id, orderNumber, storeId)
                                  .then(() => console.log('-> Pedido %i enviado id atualizado para %i', orderNumber, id))
                                current++
                                recursiveSync()
                              }
                            })

            ...
            const promise = mysql.findOrder(pedido.id, pedido.numero_ecommerce, storeId).then(async row => {
              if (row && row[0]) {
                ...
                if (orderDb.tiny_status.toLowerCase() !== pedido.situacao.toLowerCase()) {
                  ...
                  const alreadyHaveTheStatus = orderBody.fulfillments && orderBody.fulfillments.find(fulfillment => fulfillment.status !== newStatus)
                  if (typeof newStatus !== 'undefined' &&
                    orderBody.financial_status &&
                    orderBody.financial_status.current &&
                    orderBody.financial_status.current !== newStatus &&
                    !alreadyHaveTheStatus) {
                ...
                if (orderDb.tiny_status.toLowerCase() !== pedido.situacao.toLowerCase()) {
                  ...
                  if (typeof newStatus !== 'undefined' &&
                    orderBody.fulfillment_status &&
                    orderBody.fulfillment_status.current !== newStatus &&
                    !alreadyHaveTheStatus) {
                    const req = ecomClient.store({
                      ...reqConfig,

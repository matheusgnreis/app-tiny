...
module.exports = {
  ...
  updateProductQty: (sku, storeId, quantity, lastChangeBy) => {
    const sql = `UPDATE ${TB_PRODUCTS} SET updated_at = CURRENT_TIMESTAMP(), quantity = ?, last_change_by = ? WHERE sku = ? AND store_id = ?`
  ...
  updateProduct: (storeId, tinyId, ecomplusId, sku, price, title, quantity, lastChangeBy = 'ecomplus') => {
    const sql = `UPDATE ${TB_PRODUCTS} SET updated_at = CURRENT_TIMESTAMP(), tiny_id = ?, sku = ?, title = ?, price = ?, quantity = ?, last_change_by = ? WHERE ecomplus_id = ? AND store_id = ?`
  ...
  updateTinyIdProduct: (id, productId) => {
    const sql = `UPDATE ${TB_PRODUCTS} SET tiny_id = ?, updated_at = CURRENT_TIMESTAMP() WHERE id = ?`
    const values = [productId, id]
    return query(sql, values)
  },
  updateTinyIdOrder: (id, orderNumber, storeId) => {
    const sql = `UPDATE ${TB_ORDERS} SET tiny_id = ?, updated_at = CURRENT_TIMESTAMP()  WHERE order_number = ? AND store_id = ?`
  ...
  findOrder: (tinyId, orderNumber, storeId) => {
    const sql = `SELECT * FROM ${TB_ORDERS} WHERE tiny_id = ? AND order_number = ? AND store_id = ?`
    const values = [tinyId, orderNumber, storeId]
    return query(sql, values)
  },
  ...
  insertVariations: (variations, parentId, parentSku, storeId) => {
    ...
    if (variations.length) {
      variations.forEach(variation => {
        ...
        promises.push(query(sql, values))
      })
    }
  ...
  saveSync: (total, success, failed, storeId, errors) => {
    const sql = 'insert into sync_logs (total, successfully, failed, store_id, errors) values (?, ?, ?, ?, ?)'
    return query(sql, [total, success, failed, storeId, JSON.stringify(errors)])
  }
}

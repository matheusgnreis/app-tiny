...
module.exports = async (produto) => {
  ...
  schema.meta_title = produto.nome
  schema.body_html = produto.descricao_complementar
  schema.quantity = produto.saldo || 0
  ...
  if(produto.dias_preparacao) {
    schema.production_time = {}
    schema.production_time.days = produto.dias_preparacao
    schema.production_time.working_days = true
  }
...
const getBrands = (produto) => {
  ...
  if (produto.brands !== '' && produto.brands !== 'SEM MARCA') {
    brands.push({
      _id: randomObjectId(),
      name: produto.brands,
      slug: produto.brands.toLowerCase().replace(/[^\w ]+/g,'').replace(/ +/g,'-')
    })
  }
}
...
const getCategories = produto => {
  ...
  if (produto.categoria && produto.categoria !== '') {
    ...
    if (/>>/.test(categoria)) {
      ...
      categories.forEach(category => resp.push({ _id: randomObjectId(), name: category.trim(), slug: categoria.toLowerCase().replace(/[^\w ]+/g,'').replace(/ +/g,'-') }))
    } else {
      resp.push({
        name: categoria,
        _id: randomObjectId(),
        slug: categoria.toLowerCase().replace(/[^\w ]+/g,'').replace(/ +/g,'-')
      })
    }
  }
